...
GitHub Actionsで毎日2回（午前4時・午後4時 JST）、承認済みサイトのスクリーンショットを自動取得します。

### パフォーマンス最適化

- **画像サイズ**: 400x300 WebP（約15-25KB）
- **88サイト全体**: 約1.3-2.2MB
- **ページ読み込み**: 超高速（現在と同等またはそれ以上）
- **削減率**: PNG比で約93%削減
- **処理能力**: 1日最大100サイト（50サイト × 2回実行）

**比較:**
- ❌ 1200x800 PNG: 約35-44MB（非常に遅い）
- ✅ 400x300 WebP: 約1.3-2.2MB（超高速）
...
3. **スクリーンショット取得**
   - Puppeteerでブラウザを起動
   - 各サイトにアクセスして1200x800のスクリーンショット取得（高解像度）
   - 3サイトずつ並行処理

4. **Cloudinaryにアップロード**
   - 1200x800 PNG → **400x300 WebP** に自動変換
   - ファイルサイズ: 約15-25KB（超高速）
